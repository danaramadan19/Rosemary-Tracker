...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    //if i made a task and it's time is over remind me on the other day
    if (scheduledDate.isBefore(now)) {
      if (repeat == 'None') {
        scheduledDate = tz.TZDateTime(
            tz.local, now.year, now.month, (formattedDate.day) + 1, hour, minutes);
      }
// اذا فات الوقت عن الوقت يلي حددته
      if (repeat == 'Daily') {
        scheduledDate = tz.TZDateTime(
            tz.local, now.year, now.month, formattedDate.day + 1, hour, minutes);
       }
      ...
      if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(
            tz.local, now.year, now.month, (formattedDate.day) + 7, hour, minutes);
      }

      if (repeat == 'Monthly') {
        scheduledDate =   scheduledDate = tz.TZDateTime(
            tz.local, now.year, (formattedDate.month+1), formattedDate.day, hour, minutes);
       }
   ...
   print('next schad = $scheduledDate');
